// WebTransport carries confirmed controller frames. Each client simulates the
// same game locally; the relay never receives a ROM or renders the game.
...
export async function createGame(engine, config, {fetchImpl = fetch, storage = sessionStorage} = {}) {
  const {relayUrl} = await loadNetplayConfig(fetchImpl);
  const access = await readJson(await fetchImpl(`${relayUrl}/v1/rooms`, {
    method: 'POST', headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({engine, config}), credentials: 'omit',
  }));
...
export class NetplaySession {
  ...
  receive(message) {
    ...
    if (message.type === 'room') {
      ...
      if (this.started || !Number.isInteger(message.epoch) || message.epoch < 1 || message.seed !== this.room.config.seed) throw Error('Invalid game start.');
