...
type room struct {
	id, engine, address, state                  string
...
func (p *peer) send(value any) {
	...
	if p.queued.Add(int64(len(data)+1)) > 512*1024 {
		p.queued.Add(-int64(len(data) + 1))
		p.stop()
		return
	}
	select {
...
func (r *room) view() roomView {
	v := roomView{ID: r.id, Engine: r.engine, Config: append(json.RawMessage(nil), r.config...), State: r.state, Epoch: r.epoch, Seed: r.seed, ExpiresAt: r.expiresAt, Players: []playerView{}}
	for seat, p := range r.players {
		if p != nil {
			v.Players = append(v.Players, playerView{seat, p.name, p.connection != nil, p.fingerprint != ""})
		}
	}
	return v
}
...
func (h *Hub) sweepLocked() {
	...
	for id, r := range h.rooms {
		if r.state == "running" && now.Sub(r.lastFrame) > 20*time.Second {
...
func (h *Hub) Create(engine string, config json.RawMessage, name, address string) (reservation, error) {
	...
	now := h.now()
	r := &room{id: id, engine: engine, config: append(json.RawMessage(nil), config...), seed: *metadata.Seed, address: address, state: "lobby", createdAt: now, lastActive: now, expiresAt: now.Add(4 * time.Hour)}
	r.players[0] = p
	h.rooms[id] = r
...
func (h *Hub) Detach(c *peer) {
	...
	if c.seat == 0 {
		...
		if r.state == "running" || r.state == "preparing" {
...
func (h *Hub) Message(c *peer, m clientMessage) error {
	...
	case "start":
		...
		for seat, other := range r.players {
			if other != nil {
				if other.connection == nil || other.fingerprint == "" {
