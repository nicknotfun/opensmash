...
import {actions,buttonActions,connectedGamepads,defaults,eventCode,gamepadBindings,resetGamepad,familyNames,keyLabel,loadBindings,padActions,padFamily,padLabel,rebindButton,rebindKey,saveBindings,subscribeBindings,type Action,type ButtonAction,type PadFamily} from '@/lib/controls';
...
export default function Controls() {
 ...
 const mapping=gamepadBindings(bindings,profile);
 ...
 useEffect(()=>{
  ...
  const poll=()=>{
   ...
   for(const pad of connected.filter(p=>!profile||p.id===profile)){for(const a of padActions(pad,gamepadBindings(bindings,pad.id)))active.add(a);pad.buttons.forEach((b,i)=>{if(b.pressed||b.value>0.5)pressed.add(i);});}
 ...
 <label>Controller profile <select value={profile} onChange={e=>{setProfile(e.target.value);setPending(null);}}>
  ...
  {[...new Set([...pads.map(p=>p.id),...Object.keys(bindings.profiles||{})])].map(id=><option key={id} value={id}>{id}</option>)}
 </select></label>
