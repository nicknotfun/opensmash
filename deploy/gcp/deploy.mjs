...
export function checkExisting(step, resource, config, scripts) {
  ...
  if (args[0] === 'iam') {
    ...
    if (!link(resource.selfLink, `regions/${config.region}/addresses/${args[3]}`) || resource.addressType !== 'EXTERNAL' || resource.ipVersion !== 'IPV4' || resource.networkTier !== 'PREMIUM' || !isIPv4(resource.address) || (resource.users ?? []).some(user => !link(user, `zones/${config.zone}/instances/${args[3]}`))) fail();
